...
public class MapImprinter<T_Key, T_Value, T_Map extends Map<T_Key, T_Value>> extends NamedImprinter<T_Map> {
	...
	public final @NotNull AutoCoder<T_Key> keyDecoder;
	public final @NotNull AutoCoder<T_Value> valueDecoder;

	public MapImprinter(
		@NotNull ReifiedType<T_Map> type,
		@NotNull AutoCoder<T_Key> keyDecoder,
		@NotNull AutoCoder<T_Value> valueDecoder
	) {
		super(type);
	...
	public <T_Encoded> void imprint(@NotNull ImprintContext<T_Encoded, T_Map> context) throws ImprintException {
		try {
			for (Map.Entry<Data, Data> entry : context.forceAsMap().value.entrySet()) {
				T_Key key = context.fork("<key>", entry.getKey()).decodeWith(this.keyDecoder);
				T_Value value = context.fork(entry.getKey().toString(), entry.getValue()).decodeWith(this.valueDecoder);
	...
	public static class Factory extends NamedImprinterFactory {
		...
		public <T_HandledType> @Nullable AutoImprinter<?> tryCreate(@NotNull FactoryContext<T_HandledType> context) throws FactoryException {
			...
			if (keyValueTypes != null) {
				AutoCoder<?>   keyCoder = context.type(keyValueTypes[0]).forceCreateCoder();
				AutoCoder<?> valueCoder = context.type(keyValueTypes[1]).forceCreateCoder();
				return new MapImprinter<>(context.type.uncheckedCast(), keyCoder, valueCoder);
			}
			return null;
		}
	}
